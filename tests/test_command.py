...
from commoncode.system import on_linux, on_mac, on_windows, py36
...
class TestCommand(FileBasedTesting):
    ...
    def test_execute_can_handle_non_ascii_output(self):
        ...
        rc, stdout, stderr = command.execute(
            python,
            ["-c", 'print("non ascii: été just passed it !")'],
        )
        assert stderr == ""
    ...
    def test_update_path_var_on_windows(self):
        existing_path_var = "c:\\windows;C:Program Files"

        new_path = "foo\udcb1bar"
        updated_path = command.update_path_var(existing_path_var, new_path)
        assert updated_path == "foo\udcb1bar;c:\\windows;C:Program Files"
