...
from commoncode import filetype, fileutils
from commoncode.fileutils import as_posixpath
from commoncode.system import on_linux, on_mac, on_macos_14_or_higher, on_posix, on_windows
from commoncode.testcase import FileBasedTesting, make_non_executable, make_non_readable, make_non_writable
...
class TestFileUtilsIter(FileBasedTesting):
    ...
    def test_resource_iter_without_dir(self):
        test_dir = self.get_test_loc("fileutils/walk")
        base = self.get_test_loc("fileutils")
        result = sorted([as_posixpath(f.replace(base, "")) for f in fileutils.resource_iter(test_dir, with_dirs=False)])
        expected = [
            "/walk/f",
            "/walk/unicode.zip",
            "/walk/d1/f1",
    ...
    def test_resource_iter_with_dirs(self):
        test_dir = self.get_test_loc("fileutils/walk")
        base = self.get_test_loc("fileutils")
        result = sorted([as_posixpath(f.replace(base, "")) for f in fileutils.resource_iter(test_dir, with_dirs=True)])
        expected = [
            "/walk/d1",
            "/walk/d1/d2",
            "/walk/d1/d2/d3",
