...
from commoncode import fileutils, ignore
from commoncode.system import on_mac, on_windows
...
class IgnoreTest(commoncode.testcase.FileBasedTesting):
    ...
    def test_fileset_is_included_with_default_ignore_does_not_skip_one_char_names(self):
        ...
        result = [(t, fileset.is_included(t, excludes=ignore.default_ignores, includes={})) for t in tests]
        expected = [
            ("H", True),
            ("F", True),
            ("S", True),
